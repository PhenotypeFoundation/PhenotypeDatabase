...
SelectAddMore.prototype = {
    ...
    init: function(options) {
        ...
        $(that.options.selector ).find( "select[rel*='" + that.options.rel + "']").each(function() {
    ...
    addOpenDialogOption: function(element) {
        ...
        if( e.find( 'option.modify' ).length == 0 ) {
        	e.append('<option value="" class="' + style + ' modify">' + label + '</option>');
        } else {
        	// Unbind the handler if it has been added before
        	e.off( 'change' );
        }

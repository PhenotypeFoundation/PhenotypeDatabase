...
Timeline._Impl.prototype._distributeWidths = function () {
    ...
    for (var F = 0; F < this._bands.length; F++) {
        ...
        if (typeof E == "string") {
            ...
            if (F > 0) {
                var iMax = 0;
                var objElem = $("#timeline-band-" + F).find(".timeline-event-label").parent("div");
                objElem.children("div").each(function () {
                    var newPos = $(this).position().top + $(this).innerHeight();
                    //console.log(F + "            " + $(this).html() + " :: " + $(this).position().top + " :: "+ $(this).innerHeight());
                    //console.log(F + "            " + newPos + " :: " + iMax);
                    if (newPos > iMax) {
                        iMax = newPos;
                    }
                });
                D = iMax;
                //console.log(F + "            " + "new width:: "+D);
            }
        }

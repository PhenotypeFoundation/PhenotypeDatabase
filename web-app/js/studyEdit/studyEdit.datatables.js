...
StudyEdit.datatables = {
	numElements : new Array(), // Hashmap with the key being the id of the
								// table, in order to facilitate multiple tables
	elementsSelected : new Array(), // Hashmap with the key being the id of the
									// table, in order to facilitate multiple
									// tables
	tableType : new Array(), // Hashmap with the key being the id of the
								// table, in order to facilitate multiple tables
	selectType : new Array(), // Hashmap with the key being the id of the
								// table, in order to facilitate multiple tables
	allElements : new Array(), // Hashmap with the key being the id of the
								// table, in order to facilitate multiple tables
	...
	initialize : function(selector) {
		...
		$(selector).each(
				function(idx, el) {
					...
					// Initialize editable behavior, if a div with prototypes
					// exists
					...
					var defaultOptions = {
						...
						iDisplayLength : 10, // Number of items shown on one
												// page.
						aoColumnDefs : [ {
							"bSortable" : false,
							"aTargets" : [ "nonsortable" ]
						}, // Disable sorting on all columns with
							// th.nonsortable
						{
							"sSortDataType" : "formatted-num",
							"aTargets" : [ "formatted-num" ]
						} // Make sorting possible on formatted numbers
						],

						// Override the fnServerData in order to show/hide the
						// paginated
						// buttons if data is loaded
						"fnServerData" : function(sSource, aoData, fnCallback) {
							StudyEdit.datatables.retrieveData(sSource, aoData,
									fnCallback, id);
					...
					if ($el.data("datatable-options")) {
						opts = $.extend(true, {}, defaultOptions, $el
					...
					// Move messagebars from the tfoot
					var wrapper = $el.parents(".dataTables_wrapper");
					wrapper.find(".fg-toolbar:eq(1)").prepend(
							wrapper.find("tfoot .messagebar"));
	...
	removeColumnInParam : function(aoData) {
		var arrParam = new Array("bSearchable_", "sSearch_", "bRegex_",
				"bSortable_", "mDataProp_");
	 ...
	 * 
	 * initialize Initializes editable behavior on the datatable. markChanged
	 * Marks a specific input field as changed propagateChange Propagates the
	 * change in a specific input field to other selected rows. save Saves the
	 * editable form
	 **************************************************************************/
	editable : {

		/**
		 * Initializes the editable behavior on a datatable. This requires
		 * having a div with the prototypes of input fields for each field in
		 * the template. Within the div, for each element the HTML used for a
		 * field should have the id prototype_<templateId>_<fieldName>
		 */
										...
										.data(
												"datatable-options",
												{
													...
													fnDrawCallback : function() {
														// Initialize field
														// prototypes
														StudyEdit.datatables.editable.fields
																.initialize(tableId);
													},
																...
																.each(
																		...
																		function(
																			...
																			var fieldName = $(
																					...
																					.data(
																							"fieldname");
																			StudyEdit.datatables.editable.cloning
																					.cloneField(
																							tableId,
																							rowId,
																							fieldName,
		...
		cloning : {
			cloneField : function(tableId, rowId, fieldName, td, value) {
				// Check whether we have a prototype of an input for this type
				// of data
				var table = $('#' + tableId);
				var prototypeId = "#prototype_" + $(table).data("templateid")
						+ "_" + fieldName;
				var fieldPrefix = StudyEdit.datatables.editable
						.getFieldPrefix(tableId);

				// Cloning the field depends on the type: file fields are
				// handled differently
				// from other fields
				...
				if (fieldType == "FILE") {
					...
					$(td).find("div").html(
							StudyEdit.datatables.editable.cloning
									.cloneFileField(prototypeId, fieldId,
											value, rowId, tableId));
				} else {
					// Append the cloned input elements
					$(td).empty().append(
							StudyEdit.datatables.editable.cloning
									.cloneDefaultField(prototypeId, fieldId,
											value, rowId, tableId, fieldType));
				}
			},

			cloneFileField : function(prototypeId, fieldId, value, rowId,
					tableId) {
						...
						.on(
								...
								function(e) {
									...
									if (confirm("Are you sure you want to delete this file?")) {
										...
										// Change other cells as well, if this
										// row is selected
										var selectedIds = StudyEdit.datatables.elementsSelected[tableId];
										if ($.inArray(parseInt(rowId),
												selectedIds) > -1) {
											var td = $(event.target).closest(
													"td");
											StudyEdit.datatables.editable
													.propagateChangeInFileField(
															td, selectedIds);
										}
									}
									return false;
				...
				field.filter(".upload_button").attr("id",
						"upload_button_" + fieldId);

				// Update the upload icon
				field.filter(".upload_icon").attr("id",
						"upload_icon_" + fieldId);
			...
			cloneDefaultField : function(prototypeId, fieldId, value, rowId,
					tableId, fieldType) {
				...
				if (fieldInput.length > 0) {
					...
					fieldInput.attr("name", fieldId).attr("id", fieldId).val(
							value).data("original", value).data("prototype",
							prototypeId);

					StudyEdit.datatables.editable.fields.addEventsToInput(
							fieldInput, tableId, rowId);
		...
		fields : {
			// Initialize editable fields in the datatable, after they have been
			// drawn on the screen
			initialize : function(tableId) {
				...
				StudyEdit.datatables.editable.fields
						.updateAutoCompletes(prototypeSelector);

				// Initialize file upload fields
				StudyEdit.datatables.editable.fields
						.initializeFileFields(prototypeSelector);

				// Initialize ontology and template fields
				StudyEdit.datatables.editable.fields
						.initializeSelectAddMore(prototypeSelector);
			},

			/**
			 * Adds onChange and keyUp events to an input fields that handle
			 * changes and pressing enter or escape
			 */
			addEventsToInput : function(input, tableId, rowId) {
				StudyEdit.datatables.editable.fields.addChangeEventToInput(
						input, tableId, rowId);
			...
			updateAutoCompletes : function(selector) {
				$(selector).find(".ui-autocomplete-input").each(
						function(idx, el) {
							// Reinitialize autocompletes, since they break when
							// cloned. See
							// http://stackoverflow.com/questions/13664020/jquery-autocomplete-and-clone
							$(el).autocomplete(
									{
										source : $($(el).data("prototype"))
			...
			initializeSelectAddMore : function(selector) {
				StudyEdit.datatables.editable.fields
						.initializeSelectAddMoreTerms(selector);
				StudyEdit.datatables.editable.fields
						.initializeSelectAddMoreTemplates(selector);
			},
			...
			initializeSelectAddMoreTerms : function(selector) {
				new SelectAddMore().init({
					...
					onClose : function(scope) {
						StudyEdit.datatables.editable.fields.reloadPrototypes(
								selector, "term");
					}
				});
			},
			...
			/**
			 * Reloads the prototyped input fields, and propagates changes in
			 * selectboxes to the fields in the datatable. This can be used
			 * after the term-editor or tempalte-editor have been called.
			 */
				...
				$
						...
						.done(
								function(data, textStatus, jqXhr) {
									// Store the newly created prototypes of
									// select boxes in HTML
									$(data)
											.find("select[rel=" + rel + "]")
											.each(
													function(idx, field) {
									...
									// Add add/modify option again for all
									// selects
									if (rel == "template") {
										StudyEdit.datatables.editable.fields
												.initializeSelectAddMoreTemplates(selector);
		...
		markChanged : function(input) {
			...
			$(input).closest(".dataTables_wrapper").find(".saveChanges")
					.slideDown(100);
			$(input).closest(".dataTables_wrapper").find(
					".dataTables_scrollBody .dataTable").data("changed", true);
		},
		...
		save : function(link) {
			...
			wrapper.find(".changed input, .changed select, .changed textarea")
					.each(
							function(idx, el) {
								// Checkboxes must be handled differently
								if ($(el).is("[type=checkbox]")) {
									newData[$(el).attr("name")] = $(el).is(
											":checked") ? $(el).val() : "";
								} else {
			...
			// Make sure all inputs are disabled during save. When the datatable
			// is refreshed,
		...
		discardChanges : function(link) {
			...
			// Set the 'changed' flag to false
			wrapper.find(".dataTables_scrollBody .dataTable").data("changed",
					false);
		...
		showError : function(tableId, message, details) {
			...
			div.append($("<a>").attr("href", "#").addClass("close")
			...
			var container = $("<div>").addClass("messagebar").addClass("errorbar")
					.append(div);
	...
	/***************************************************************************
	 * 
	 * These function are used for handling selectboxes and select-all boxes. In
	 * fact, there are four methods:
	 * 
	 * checkSelectedCheckboxes checks selectboxes based on the ids previously
	 * selected (when showing a new page in a serverside paginated table)
	 * checkAllPaginated (cs & ss) handles a click on the 'checkAll' button:
	 * checks all items if not all items were selected, and deselects all items
	 * if all items were selected updateCheckAll (cs & ss) updates the checkAll
	 * checkbox so it shows the current status: checked if everything is
	 * selected, checked but transparent if some items are selected and
	 * deselected if no items are selected submitPaginatedForm submits a form
	...
	selection: {
		...
		initialize : function(selector) {
				...
				$("#" + id + ' tbody tr')
						.each(
								function(idxrow, row) {
									...
									var input = $("<input id='"
											+ id
											+ "_ids' type='checkbox' value='"
											+ rowid
											+ "' class='selectable-datatable-checkbox' name='"
											+ id + "_ids'>");
									input.on("click", function() {
										StudyEdit.datatables.selection
												.clickRow(this);
									});
									$(row).prepend(
				...
				$("#" + id + ' tbody tr')
						.each(
								function(idxrow, row) {
									...
									var input = "<input id='"
											+ id
											+ "_ids' type='radio' class='selectable-datatable-checkbox' value='"
											+ rowid + "' name='" + id
											+ "_ids'>";
									input.on("click", function() {
										StudyEdit.datatables.selection
			...
			$el.find(".selectAll a").on("click", function() {
				StudyEdit.datatables.selection.selectAll(id);
				return false;
				$(this).closest(".selectAll").slideUp(100);
			});
		...
		clickRow : function(inputrow) {
			...
			var paginatedTable = wrapper
					.find(".dataTables_scrollBody .dataTable");
			var dataTable = wrapper;
			var tableId = paginatedTable.attr('id');
			var checked = input.attr('checked');

			// If the input is a normal checkbox, the user clicked on it. Update
			// the elementsSelected array
			if (StudyEdit.datatables.selectType[tableId] == "selectMulti") {
				...
				StudyEdit.datatables.selection.select(tableId, parseInt(input
						.val()), checked);
				StudyEdit.datatables.selection.updateCheckAll(inputrow);
			} else {
				// Assumption: selectType[ tableId ] == "selectOne"
				StudyEdit.datatables.elementsSelected[tableId][0] = parseInt(input
						.val());
			}

			// Disable the 'select all pages' button
			wrapper.find(".selectAll").slideUp(100);
		},
		...
		select : function(tableId, id, flag) {
			var arrayPos = jQuery.inArray(id,
					StudyEdit.datatables.elementsSelected[tableId]);

			if (flag) {
				// Put the id in the elementsSelected array, if it is not
				// present
				if (arrayPos == -1) {
					StudyEdit.datatables.elementsSelected[tableId][StudyEdit.datatables.elementsSelected[tableId].length] = id;
				}
			} else {
				// Remove the id from the elementsSelected array, if it is
				// present
				if (arrayPos > -1) {
					StudyEdit.datatables.elementsSelected[tableId].splice(
							arrayPos, 1);
				}
			}
		},
		...
		clickCheckAll : function(input) {
			...
			var inputsOnScreen = $('tbody input.selectable-datatable-checkbox',
					paginatedTable);
			...
			if (checkAll.attr('checked')) {
				...
				for ( var i = 0; i < inputsOnScreen.length; i++) {
					...
					if (!input.hasClass("checkall")) {
						...
						StudyEdit.datatables.elementsSelected[tableId][StudyEdit.datatables.elementsSelected[tableId].length] = parseInt(input
								.val());
					}
				}

				// Show a message whether the user wants to select all items on
				// all pages
				if (typeof (StudyEdit.datatables.allElements[tableId]) != "undefined"
					...
					wrapper.find(".selectAll").slideDown(100);
				}
				...
				for ( var i = 0; i < inputsOnScreen.length; i++) {
					var input = $(inputsOnScreen[i]);
					if (!input.hasClass("checkall")) {
						var arrPos = jQuery.inArray(parseInt(input.val()),
								StudyEdit.datatables.elementsSelected[tableId]);
						if (arrPos > -1) {
			...
			// The ui-selected class is used by the selectable element, and is
			// not needed
			// for the internal selection method. If the classes remain on the
			// row, the checkbox
			// will be checked again after selecting one or more rows with
			// selectable.
		...
		selectAll : function(tableId) {
			if (typeof (StudyEdit.datatables.allElements[tableId]) != "undefined") {
				...
				var checkAll = $('#' + tableId + '_checkAll', wrapper);
				var inputsOnScreen = $(
						'tbody input.selectable-datatable-checkbox',
						paginatedTable);

				// Add elements to internal storage
				StudyEdit.datatables.elementsSelected[tableId] = StudyEdit.datatables.allElements[tableId]
						.slice(0);
		...
		/**
		 * Update the status of the checkAll button, after an element has been
		...
		/**
		 * Updates the label underneath the table, specifying how many items are
		 * selected
		 */
		updateLabel : function(tableId) {
			if (StudyEdit.datatables.elementsSelected[tableId].length > 0) {
				$("#" + tableId + "_selectinfo")
						.text(
								" ("
										+ StudyEdit.datatables.elementsSelected[tableId].length
										+ " selected)")
						.append(" (")
						.append(
								$("<a href='#'></a>")
		...
		checkSelectedCheckboxes : function(tableId) {
			...
			for ( var i = 0; i < trsOnScreen.length; i++) {
				...
				// Only add the input if the list is not empty. The list is
				// empty if
				...
				if (!td.hasClass('dataTables_empty')) {
					...
					var strChecked = "";
					if (jQuery.inArray(parseInt(rowid),
							StudyEdit.datatables.elementsSelected[tableId]) > -1) {
						strChecked = " CHECKED ";
					}

					// Add a radio button for selectOnce and a checkbox for
					// selectMulti
					var strType = "radio";
					if (StudyEdit.datatables.selectType[tableId] == "selectMulti") {
						strType = "checkbox";
					...
					// Replace the current contents of the cell with the newly
					// created input field
					var input = $("<input id='" + tableId + "_ids_" + rowid
							+ "' class='selectable-datatable-checkbox' type='"
							+ strType + "' value='" + rowid + "' name='"
							+ tableId + "_ids'" + strChecked + ">");
					input.on("click", function() {
		...
		/**
		 * Submit a form taking into account its paginated nature (i.e. it has
		 * been spread over multiple pages)
		 */
		...
		/**
		 * Initializes jQuery selectable on the datatable, so the user can
											...
											.each(
													function() {
														...
														if (id
															...
															var input = $(this)
																	.find(
																			"input.selectable-datatable-checkbox");
															if (input.length > 0) {
																input
																		.attr(
																				"checked",

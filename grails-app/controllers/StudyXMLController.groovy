...
class StudyXMLController {
	...
	def index = {
		...
		def list = exportService.getDependentObjects( Study.get(1) ).unique() 

		list*.class.unique().each{ println "found: ${it}" }
	...
	/** Some domain objects require special tweaking by hand before they 
	  ...
	  * identified by a given externalAssayID.
	  * 
	...
	def treatConstraints( List parseObjects ) {
		...
		parseObjects.each { it ->
			...
			switch( domainObject ) {
				case Assay: 
							// if assay with externalAssayID exists in database, 
							// this object is replaced by the object in the database.
					def existingAssay = Assay.findByExternalAssayID( domainObject.externalAssayID )
    ...
    /**  @param 
	  ...
	  *  
	  *  To do: this could become a member method of ParseObject.
	  */

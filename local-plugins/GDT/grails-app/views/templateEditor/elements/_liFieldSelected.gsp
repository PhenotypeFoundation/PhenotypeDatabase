<span class="listButtons">
  <img onClick="showTemplateFieldForm( ${templateField.id}); this.blur(); return false;" src="${resource( dir: 'images/icons', file: 'application_edit.png', plugin: 'famfamfam' )}" alt="Show template field properties" title="Show template field properties">
      <g:if test="${template.entity != org.dbxp.sam.Feature && templateField.isFilledInTemplate(template)}">
  ...
  <form class="templateField_form" id="templateField_${templateField.id}_form" action="updateField">
      <g:if test="${template.entity == org.dbxp.sam.Feature || templateField.isEditable()}">

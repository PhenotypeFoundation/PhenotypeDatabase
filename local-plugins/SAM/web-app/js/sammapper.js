//TODO: merge with 'samimporter.js' into a single function (url and message as params)
//TODO: add 'listFeatures.gsp' javascript functions
...
function insertOnRedirectWarning() {
    ...
    $('a').each(function() {
        ...
        // bind to the anchor?
        if (!element.attr('href').match(/^#/gi) && !element.attr('href').match(/SAMIdmapper/gi)) {
...
function onDirectWarning() {
    return confirm('Warning: navigating away from the mapper might cause loss of work and unsaved data. Are you sure you want to continue?');
}

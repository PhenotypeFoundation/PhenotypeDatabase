...
class FuzzySearchService {
    ...
    static def stringSimilarity (l_str, r_str, degree=2) {
        def leftString = l_str.toString().toLowerCase()
        def rightString = r_str.toString().toLowerCase()
        
        // Handle special case of perfect match
        if( l_str.trim() == r_str.trim() )
            return Float.MAX_VALUE
        
        similarity(leftString.toCharArray(),
                rightString.toCharArray(),
                degree)
    }
